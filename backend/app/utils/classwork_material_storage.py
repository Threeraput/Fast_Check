...
# backend/app/utils/classwork_material_storage.py -> parents[2] == backend/
_BACKEND_ROOT: Final[Path] = Path(__file__).resolve().parents[2]
_UPLOAD_DIR: Final[Path] = _BACKEND_ROOT / "uploads" / "classwork" / "materials"
...
async def save_material_file(file: UploadFile) -> tuple[str, str, int]:
    ...
    relative_path = f"uploads/classwork/materials/{filename}"
    return relative_path, mime, size


def delete_material_file(storage_path: str) -> None:
    if not storage_path:
        return
