...
from app.models.association import (
    user_roles,
    class_students,
)  # ตรวจสอบว่า import class_students ด้วย
...
class User(Base):
    ...
    location_updates = relationship(
        "TeacherLocation", back_populates="teacher", cascade="all, delete-orphan"
    )
    student_locations = relationship(
        "StudentLocation", back_populates="student", cascade="all, delete-orphan"
    )
    # User -> Face Samples (One-to-Many)
    face_samples = relationship(
        "UserFaceSample", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    class_assignments = relationship(
        "ClassworkAssignment", back_populates="teacher", cascade="all, delete-orphan"
    )
    class_submissions = relationship(
        "ClassworkSubmission", back_populates="student", cascade="all, delete-orphan"
    )
